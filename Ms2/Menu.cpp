...
#ifdef _MSC_VER
            strcpy_s(m_content, strlen(content) + 1, content); // For MSVC
#else
            std::strncpy(m_content, content, strlen(content) + 1); // Portable alternative
            m_content[strlen(content)] = '\0'; // Ensure null-termination
#endif
        }
    }
